...
You can install Mint Tasks either as a system-wide Debian package (`.deb`) or via the user-space installer script.

### Method A: Native `.deb` Package (System-Wide)

Build or download the standalone `mint-tasks.deb` package:
...
# Install using apt or gdebi
sudo apt install ./mint-tasks_1.0.0_amd64.deb
```

Installed to `/opt/mint_tasks/` with `/usr/bin/mint-tasks`, launcher, and scalable icon.

---

### Method B: User-Space Installer (No `sudo` needed)
